          ...
          <div className="mt-1 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden z-10 relative">
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-sm border-b border-slate-50 last:border-b-0 transition-colors ${activeTab === tab.id ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-600 hover:bg-slate-50'
                  }`}
              >
                <span>{tab.icon}</span>
              ...
              <div className="grid grid-cols-3 gap-1">
                {[
                  ...
                  <button
                    ...
                    className={`py-1.5 rounded-lg text-xs font-bold transition-all ${lang === l.code
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5">
          ...
          <button
            onClick={onSave}
            disabled={loading}
            className={`flex-shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white transition-all shadow-sm ${saved ? 'bg-emerald-500' : 'bg-gradient-to-r from-indigo-500 to-purple-600 hover:opacity-90'
              } disabled:opacity-60`}
          >
            {loading ? (
        ...
        <div className="mt-4 grid grid-cols-3 sm:grid-cols-6 gap-2">
          {languages.map(l => (
            <button
              ...
              className={`py-2.5 rounded-xl text-xs font-bold transition-all border-2 ${profileData.language === l.code
        ...
        <div className="flex flex-wrap gap-2">
          {freqKeys.map((key, i) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold border-2 transition-all ${prefs.digest_frequency === freqVals[i]
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {providers.map(p => (
            <button key={p.id} onClick={() => setProvider(p.id)}
              className={`flex items-center gap-2 p-3 sm:p-4 rounded-xl border-2 transition-all text-left ${provider === p.id ? 'border-indigo-400 bg-indigo-50' : 'border-slate-100 hover:border-slate-200 bg-white'
                }`}>
            ...
            {integrations.filter(i => i.category === cat).map(integration => (
              <div key={integration.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sm:p-5 flex items-center justify-between gap-3 sm:gap-4">
                ...
                <button
                  ...
                  className={`flex-shrink-0 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold border-2 transition-all ${connected[integration.id]
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {plans.map(plan => (
            <div key={plan.id} className={`relative p-4 sm:p-5 rounded-xl border-2 transition-all ${currentPlan === plan.id ? 'border-indigo-400 bg-indigo-50' : 'border-slate-100 hover:border-slate-200'
              }`}>
              {plan.popular && (
              ...
              <button disabled={currentPlan === plan.id}
                className={`w-full py-2 rounded-lg text-sm font-bold transition-all ${currentPlan === plan.id ? 'bg-indigo-100 text-indigo-600 cursor-default' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                  }`}>
