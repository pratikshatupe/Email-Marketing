...
const PLANS = [
  {
    ...
    max_emails_per_month: 1000, max_contacts: 500,
    features: ['Basic Templates', 'Email Support'],
  },
  {
    id: 2, name: 'Basic', price: 499, duration: 30,
    max_emails_per_month: 10000, max_contacts: 2000,
    features: ['All Templates', 'Priority Support', 'Analytics'],
  },
  {
    id: 3, name: 'Pro', price: 999, duration: 30,
    max_emails_per_month: 50000, max_contacts: 10000,
    features: ['All Templates', '24/7 Support', 'Advanced Analytics', 'Automation'],
  },
  {
    id: 4, name: 'Enterprise', price: 2499, duration: 30,
    max_emails_per_month: 200000, max_contacts: 100000,
    features: ['Custom Templates', 'Dedicated Manager', 'Full Analytics', 'Automation', 'API Access'],
  },
];
...
export default function Subscription() {
  ...
  function handleAssign() {
    if (!assignData.user_id || !assignData.plan_id) {
      showToast('User ID aur Plan dono bharo.', 'error');
      return;
    }
    ...
    showToast('Plan successfully assign kiya gaya!');
  }
  ...
  function handleCancel(id) {
    ...
    showToast('Subscription cancel kar diya gaya.');
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Toast */}
      {toast && (
        <div className={`rounded-xl px-4 py-3 text-sm border flex items-center justify-between ${
          toast.type === 'error'
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white">
        ...
        <div className="flex flex-wrap gap-6 mt-3 text-sm">
          <div>
            ...
            <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-green-400/20 text-green-100">
              {MY_SUB.status}
            </span>
          </div>
          <div>
            <span className="text-indigo-200">Max Contacts: </span>
            <span className="font-medium">{MY_SUB.max_contacts.toLocaleString()}</span>
          </div>
          <div>
            <span className="text-indigo-200">Emails/Month: </span>
            <span className="font-medium">{MY_SUB.max_emails_per_month.toLocaleString()}</span>
          </div>
          <div>
            <span className="text-indigo-200">Expires: </span>
            <span className="font-medium">{new Date(MY_SUB.end_date).toLocaleDateString('en-IN')}</span>
          </div>
        </div>
      </div>
              ...
              <tbody>
                {subs.map(sub => (
                  <tr key={sub.id} className="border-b border-slate-50 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      {sub.status === 'active' && (
                        <button onClick={() => handleCancel(sub.id)} className="text-red-500 hover:text-red-700 text-xs font-medium">
                          Cancel
                        </button>
                      )}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
                ...
                >
                  ...
                  {PLANS.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.name} — ₹{p.price.toLocaleString()} ({p.duration} days)
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="p-6 border-t border-slate-100 flex justify-end gap-3">
              <button onClick={() => setAssignModal(false)} className="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                Cancel
              </button>
              <button onClick={handleAssign} className="px-5 py-2 text-sm font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-colors">
                Assign Karein
              </button>
            </div>
          </div>
