...
const mockCampaigns = [
  {
    ...
    subject: "☀️ Exclusive Summer Deals — Up to 50% Off!",
    template_name: "Discount Template",
    segment_name: "Newsletter Subscribers",
    status: "sent",
    total_sent: 5000,
    scheduled_at: "2026-03-20T10:00",
    created_at: "2026-03-18T09:00",
  },
  {
    id: 2,
    name: "Product Launch",
    ...
    status: "scheduled",
    total_sent: 0,
    scheduled_at: "2026-04-01T09:00",
    created_at: "2026-03-25T11:00",
  },
  {
    id: 3,
    name: "Re-engagement Campaign",
    subject: "We Miss You! Here's 20% Off",
    template_name: "Winback Template",
    segment_name: "Returning Customers",
    status: "draft",
    total_sent: 0,
    scheduled_at: null,
    created_at: "2026-03-26T14:00",
  },
  {
    id: 4,
    name: "Welcome Series",
    subject: "👋 Welcome — Get Started with Us",
    template_name: "Welcome Template",
    segment_name: "New Customers",
    status: "pending_approval",
    total_sent: 0,
    scheduled_at: null,
    created_at: "2026-03-27T10:00",
  },
];
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-6 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Admin Dashboard →  go to the Campaigns </p>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="space-y-2 mb-5">
            ...
            {[
              { icon: "✍️", label: "Draft",            desc: "Tumchi current status",    active: true,  done: true  },
              { icon: "⏳", label: "Pending Approval",  desc: "Admin review karil",        active: true,  done: false },
              { icon: "✅", label: "Approved / Scheduled", desc: "Campaign queue madhe",   active: false, done: false },
              { icon: "📤", label: "Emails Sent",       desc: "Contacts cha inbox la janar", active: false, done: false },
            ].map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0
                  ${step.done ? "bg-green-100" : step.active ? "bg-indigo-100" : "bg-gray-100"}`}>
                  {step.icon}
                </div>
                <div className="flex-1">
                  <span className={`text-xs font-semibold ${step.active ? "text-indigo-700" : "text-gray-500"}`}>
                    {step.label}
                  </span>
                  <span className="text-xs text-gray-400 ml-2">— {step.desc}</span>
                </div>
                {i < 3 && <div className="w-px h-3 bg-gray-200 absolute ml-3.5 mt-7" />}
              </div>
            ))}
          </div>

          <p className="text-xs text-gray-500 bg-yellow-50 border border-yellow-100 rounded-lg px-3 py-2">
            ⚠️ "After submitting, the campaign will appear in the Admin Dashboard under the Campaigns section, where the admin can approve or reject it."
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {isEdit ? " update  Details " : " create Bulk email campaign "}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              {form.subject.length}/150 chars — Emoji use karne se open rate badhti hai 📈
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-400 mt-1">"If left empty, it will be saved as a draft."</p>
          </div>
        </div>
...
export default function EmailCampaigns() {
  ...
  const [editTarget,   setEditTarget]   = useState(null);   // campaign object to edit
  const [deleteTarget, setDeleteTarget] = useState(null);   // campaign object to delete
  const [submitTarget, setSubmitTarget] = useState(null);   // campaign object to submit
...
addToast("Campaign successfully saved as a draft! ✍️", "success");  };
  ...
  const handleSubmit = () => {
    ...
    addToast(
`${submitTarget.name} has been submitted to the Admin Dashboard → Campaigns. The admin will review it. ⏳`,      "info"
    );
  ...
  const handleApprove = (campaign) => {
    ...
    addToast(`"${campaign.name}" approved! ${campaign.scheduled_at ? "Scheduled ✅" : "Sending start 📤"}`, "success");
  };
  ...
  const handleSendNow = (campaign) => {
    ...
    addToast(`"${campaign.name}" sending is start ! 📤`, "info");
  };
  ...
  /* ─────────────────── ACTION BUTTONS (reused in table & cards) ─────────────────── */
    ...
    <div className={`flex items-center gap-1.5 ${compact ? "" : "justify-end"}`}>
      ...
      {c.status === "draft" && (
        <button
          ...
          className="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-semibold hover:bg-indigo-100 transition-colors whitespace-nowrap"
        >
          Submit
        </button>
      )}
      ...
      {c.status === "pending_approval" && (
        <>
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-red-50 text-red-700 text-xs font-semibold hover:bg-red-100 transition-colors"
          >
            Reject
          </button>
        </>
      ...
      {c.status === "scheduled" && (
        <button
          ...
          className="px-2.5 py-1 rounded-lg bg-yellow-50 text-yellow-700 text-xs font-semibold hover:bg-yellow-100 transition-colors whitespace-nowrap"
        >
          Send Now
        </button>
      )}
      ...
      {/* View Analytics */}
      {c.status === "sent" && (
        <button className="px-2.5 py-1 rounded-lg bg-gray-50 text-gray-600 text-xs font-semibold hover:bg-gray-100 transition-colors whitespace-nowrap">
          Analytics
        </button>
      )}

      {/* Edit button — only for non-sent/sending */}
      ...
      {/* Delete button — only for non-sent/sending */}
      {!["sent", "sending"].includes(c.status) && (
        <button
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Email marketing campaigns manage karo</p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Total Campaigns",   value: stats.total,                    icon: "📧", sub: "sabhi campaigns" },
            ...
            { label: "Pending Approval",  value: stats.pending,                  icon: "⏳", sub: "review pending" },
          ].map(s => (
...
placeholder="Search campaign or subject..."
          ...
          <div className="flex gap-1 bg-white rounded-xl border border-gray-200 p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${viewMode === "table" ? "bg-blue-600 text-white" : "text-gray-500 hover:text-gray-700"}`}
            >
              Table
            </button>
            <button
              onClick={() => setViewMode("card")}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${viewMode === "card" ? "bg-blue-600 text-white" : "text-gray-500 hover:text-gray-700"}`}
            >
              Cards
            </button>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
            ...
            <p className="text-gray-500 text-sm">campagins are not found </p>
            <button
              onClick={() => setShowCreate(true)}
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map((c, i) => (
                    <tr
                      key={c.id}
                      className={`border-b border-gray-50 hover:bg-gray-50/60 transition-colors ${i === filtered.length - 1 ? "border-b-0" : ""}`}
                    >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(c => (
              <div key={c.id} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow flex flex-col">
                ...
                <div className="space-y-2 mb-4 flex-1">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <span className="text-gray-400">📤</span>
                    <span>{c.total_sent.toLocaleString()} emails sent</span>
                  </div>
                  {c.scheduled_at && (
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <span className="text-gray-400">🕐</span>
                      <span>{new Date(c.scheduled_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}</span>
                    </div>
                  )}
                </div>
